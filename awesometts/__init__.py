...
# import Log
# logger = Log.getLogger("tts")

sequences = {key: QKeySequence()
             for key in ['browser_generator', 'browser_stripper',
                         'configurator', 'editor_generator', 'templater', 'custom_generator']}
...
config = Config(
    ...
    cols=[
        ...
        ('launch_browser_stripper', 'integer', None, to.nullable_key,
         to.nullable_int),
        ('launch_custom_generator', 'integer', None, to.nullable_key,
         to.nullable_int),
...
def browser_menus():
    ...
    def on_setup_menus(browser):
        ...
        gui.Action(
            target=Bundle(
                constructor=gui.CustomGenerator,
                args=(),
                kwargs=dict(browser=browser,
                            addon=addon,
                            alerts=aqt.utils.showWarning,
            ...
            text="&Custom add multi Audios from Selected...",
            sequence=sequences['custom_generator'],
            parent=menu,
        )
